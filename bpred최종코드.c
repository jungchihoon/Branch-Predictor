	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    /* bimodal component */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred2bit, 2097152, 0, 0, 0);

    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, 1, 268435456, 28, 1);
					 ...
					 * used on mispredict recovery */
{
  ...
  index = (baddr >> MD_BR_SHIFT) & (pred->btb.sets - 4);
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (taken)
    {
      index = (baddr >> MD_BR_SHIFT) & (pred->btb.sets - 2);
